...
async def on_message(message):
    if client.user != message.author:    # 送り主がBotだった場合反応したくないから
        if message.content.startswith('/help'):   # 改行コード使用例↓ と、代入するのはmじゃなくてもいい証明
            desc1 = '\n・おはよう\n・おやすみ\n(・かわいい)\n・生きてる\n・おみくじ\n(・今日の格言)\n・鈴花'
            embed = discord.Embed(title="反応する言葉",description=desc1, color=0x64ec86)    # embedオブジェクトの定義
            embed.add_field(name="/say",value="何かを言わせられるよ",inline=False)    # フィールドの追加　横に並べたいときはinlineをtrueに
        ...
        elif message.content.startswith('おはよう'):    # どんな言葉で始まるか調べる
